...
struct cmc623_data {
	...
	struct clk *clk;
	struct mutex tuning_mutex; /* config registers lock */
	bool suspended;
	bool initialized;
...
static int cmc623_brightness_to_intensity(struct i2c_client *client, int brightness);
...
static int cmc623_write_reg(struct i2c_client *client, unsigned char addr, unsigned int data)
{
	...
	if (err < 0) {
		dev_err(&client->dev, "i2c_transfer failed. err %d, addr %x, data %x\n",
			err, addr, data);
		return err;
	}
...
static int cmc623_get_brightness(struct backlight_device *backlight)
{
	...
	return cmc623_brightness_to_intensity(client, backlight->props.brightness);
}
...
static const struct cmc623_gpio_init cmc623_gpio_init_table[] = {
...
static int cmc623_init_gpios(struct i2c_client *client, struct cmc623_data *data)
{
...
static int cmc623_i2c_probe(struct i2c_client *client)
{
	...
	data->backlight = backlight_device_register("pwm-backlight", &client->dev,
						    data, &cmc623_backlight_ops, &props);
	...
	cmc623_resume(client);
	intensity = cmc623_brightness_to_intensity(client, data->backlight->props.brightness);
	cmc623_set_backlight(client, intensity);
