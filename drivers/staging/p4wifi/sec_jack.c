...
static unsigned int p4wifi_jack_determine_type(struct jack_info *info)
{
	...
	unsigned int npolarity = !pdata->det_active_high;
...
static void p4wifi_jack_buttons_work(struct work_struct *work)
{
...
static int p4wifi_jack_parse_dt(struct platform_device *pdev,
				struct jack_platform_data *pdata)
{
	...
	pdata->zones = devm_kzalloc(dev, sizeof(struct jack_detect_zone) *
				    pdata->num_zones, GFP_KERNEL);
	...
	if (!pdata->num_buttons_zones)
		dev_err(dev, "no jack button zones specified\n");

	pdata->buttons_zones = devm_kzalloc(dev, sizeof(struct jack_button_zone) *
					    pdata->num_buttons_zones, GFP_KERNEL);
...
static int p4wifi_jack_probe(struct platform_device *pdev)
{
	...
	pdata = devm_kzalloc(dev, sizeof(struct jack_platform_data), GFP_KERNEL);
	if (!pdata)
		return -ENOMEM;
	...
	info->adc_channel = devm_iio_channel_get(&pdev->dev, "headset-jack-detect");
...
static void p4wifi_jack_remove(struct platform_device *pdev)
{
