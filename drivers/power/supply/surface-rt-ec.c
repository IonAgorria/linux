...
static int srt_ec_probe(struct i2c_client *client)
{
	...
	err = devm_delayed_work_autocancel(dev, &srt->poll_work,
					   srt_bat_poll_work);
	if (err)
		return err;
...
static int __maybe_unused srt_ec_suspend(struct device *dev)
{
...
static int __maybe_unused srt_ec_resume(struct device *dev)
{
	struct srt_ec_device *srt = dev_get_drvdata(dev);
...
static SIMPLE_DEV_PM_OPS(srt_ec_pm_ops, srt_ec_suspend, srt_ec_resume);
